...
class VirusClassifier(nn.Module):
    def __init__(self):
        ...
        self.scconv1 = ScConv(op_channel=32, group_num=4, gate_treshold=0.5, alpha=1/2, squeeze_radio=2, group_size=2, group_kernel_size=3)
        self.ca1 = CoordAtt(32, 32)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2 = nn.Conv2d(32, 64, 3, padding=1)
        self.scconv2 = ScConv(op_channel=64, group_num=4, gate_treshold=0.5, alpha=1/2, squeeze_radio=2, group_size=2, group_kernel_size=3)
        self.ca2 = CoordAtt(64, 64)
        self.pool2 = nn.MaxPool2d(2, 2)

        self.conv3 = nn.Conv2d(64, 128, 3, padding=1)
        self.scconv3 = ScConv(op_channel=128, group_num=4, gate_treshold=0.5, alpha=1/2, squeeze_radio=2, group_size=2, group_kernel_size=3)
...
def main():
    data_path = r"/home/zhangjianpeng/Virus_identity/data/pig/val"  # 替换为数据文件夹路径
    batch_size = 1
    ...
    model_classifier.load_state_dict(
        torch.load(r"/home/zhangjianpeng/Virus_identity/model/pig.pth")
    )
    ...
    criterion = nn.BCEWithLogitsLoss()  # 假设这是一个二分类任务
    ...
    for batch_idx, dataset in enumerate(custom_loader):
        ...
        with torch.no_grad():
            for batch_features, batch_embeddings, batch_labels, batch_ids in val_loader:
                ...
                # 获取预测概率
                prob_class1 = torch.sigmoid(outputs).detach().cpu().numpy().flatten() # 类别1的概率
