...
class CombinedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            # 由于 transforms 期望的是 PIL 图像或特定格式的 Tensor，确保 feature 格式兼容
            # 如果 feature 是 Tensor，可以直接应用
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        print(f"验证损失下降，保存模型到 {self.path}")
...
class VirusClassifier(nn.Module):
    def __init__(self):
        ...
        self.scconv1 = ScConv(op_channel=16, group_num=4, gate_treshold=0.5, alpha=1 / 2, squeeze_radio=2, group_size=2,
                              group_kernel_size=3)
        self.ca1 = CoordAtt(16, 16)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2 = nn.Conv2d(16, 32, 3, padding=1)
        self.scconv2 = ScConv(op_channel=32, group_num=4, gate_treshold=0.5, alpha=1 / 2, squeeze_radio=2, group_size=2,
                              group_kernel_size=3)
        self.ca2 = CoordAtt(32, 32)
        self.pool2 = nn.MaxPool2d(2, 2)

        self.conv3 = nn.Conv2d(32, 64, 3, padding=1)
        self.scconv3 = ScConv(op_channel=64, group_num=4, gate_treshold=0.5, alpha=1 / 2, squeeze_radio=2,
        ...
        self.attention_embed = nn.MultiheadAttention(embed_dim=256, num_heads=8)
        self.dropout_embed1 = nn.Dropout(0.5)
        self.fc_embed2 = nn.Linear(256, 128)
        self.dropout_embed2 = nn.Dropout(0.5)

        # 融合后全连接层
        self.fc_combined1 = nn.Linear(128 + 128, 128)
        self.dropout_combined1 = nn.Dropout(0.5)
        self.fc_out = nn.Linear(128, 1)  # 二分类
    ...
    def forward(self, feature_matrix, embedding_vector):
        ...
        y = y.unsqueeze(1)  # 添加序列长度维度，形状变为 [batch_size, 1, 256]
        y, _ = self.attention_embed(y, y, y)  # 自注意力
        y = y.squeeze(1)  # 移除序列长度维度，形状恢复为 [batch_size, 256]
...
def main():
    ...
    data_path = r"/home/zhangjianpeng/Virus_identity/data/pytorch_dataset_100-400"
    ...
    for file in all_files:
        ...
        try:
            data = torch.load(file_path)
            labels.append(Counter(data['labels']).most_common(1)[0][0] if data['labels'] else None)
        except Exception as e:
            print(f"错误: 加载文件 {file} 时出错: {e}")
            labels.append(None)

    # 移除标签为2的文件
    filtered_files = [f for f, label in zip(all_files, labels) if label is not None and label != 2]
    filtered_labels = [label for label in labels if label is not None and label != 2]

    # 使用train_test_split进行分层划分，确保训练集和验证集中的标签分布一致
    ...
    sorted_val_files = sorted(val_files, key=extract_number)
    print("\n验证集文件名列表:")
    for file in sorted_val_files:
        print(file)
    ...
    train_dataset = CombinedDataset(data_path, train_files, transform=train_transform)
    val_dataset = CombinedDataset(data_path, val_files)  # 验证集不进行数据增强
    ...
    optimizer = optim.AdamW(model.parameters(), lr=5e-5, weight_decay=5e-3)  # 调整学习率和权重衰减
    ...
    for epoch in range(num_epochs):
        ...
        try:
            ...
            auc = 0.0  # 如果只有一个类别，则ROC AUC无法计算

        # 计算混淆矩阵
        cm = confusion_matrix(val_targets, val_preds)
        if cm.shape == (2, 2):
            TN, FP, FN, TP = cm.ravel()
            sensitivity = TP / (TP + FN) if (TP + FN) > 0 else 0.0  # Sn
            specificity = TN / (TN + FP) if (TN + FP) > 0 else 0.0  # Sp
        else:
            # 如果只有一个类别存在，设定敏感性和特异性为0
            sensitivity = 0.0
            specificity = 0.0

        print(f"\n验证损失: {val_loss:.3f}")
        print(f"验证准确率 (ACC): {val_accuracy:.3f}")
        print(f"验证敏感性 (Sn): {sensitivity:.3f}")
        print(f"验证特异性 (Sp): {specificity:.3f}")
        print(f"验证F1分数: {f1:.3f}")
        print(f"验证AUC (ROC): {auc:.3f}")
