...
def extract_sort_key(filename):
    # 提取类别
    category_match = re.search(r'(Prokaryotes Virus|Eukaryotes Virus)', filename)
    category = category_match.group(1) if category_match else 'Unknown'
    
    # 提取 Virus 后的数字
    number_match = re.search(r'Virus(\d+)', filename)
...
def main():
    ...
    data_path = r"/home/zhangjianpeng/Virus_identity/data/pig/pytorch"
    ...
    # 添加以下部分以打印验证集的文件名
    sorted_val_files = sorted(val_files, key=extract_sort_key)
    print("\n验证集文件名列表:")
    for file in sorted_val_files:
        print(file)
    # 创建训练和验证的 CombinedDataset
    train_dataset = CombinedDataset(data_path, train_files)
    val_dataset = CombinedDataset(data_path, val_files)
    ...
    # 如果存在类别不平衡，使用 WeightedRandomSampler 已经处理
    # 计算权重
    # 移除 pos_weight
    ...
    early_stopping = EarlyStopping(patience=20, delta=0.001, path=best_model_path)
    ...
    for epoch in range(num_epochs):
        ...
        # 早停机制
        if early_stopping.step(epoch_loss, model):
            break
